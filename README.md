...
- **病虫害检测**：支持单图检测、批量检测、视频检测、摄像头实时检测四种模式
- **视频分析**：自动抽帧检测，IoU 跨帧去重追踪，生成标注视频和病虫害统计摘要
- **摄像头实时检测**：EMA 平滑框绘制，累积物种清单，5 秒更新
- **检测结果图鉴**：清单中点击物种弹出浮窗卡片，支持跳转图鉴页查看详情
- **检测历史**：记录所有检测结果，视频封面自动提取关键帧
- **病虫害图鉴**：67/102 类常见农作物病虫害的详细资料
- **数据看板**：个人检测统计、今日平台活跃数据
- **用户管理**：admin/user 两级权限，支持角色切换、账户启禁用
- **UI 动效**：全局统一动效系统，渐进式入场、悬浮反馈

## 技术栈

- **后端**：Python FastAPI + PostgreSQL + YOLO (ultralytics)
- **前端**：Vue 3 + Vite + Element Plus + Pinia
- **AI 模型**：YOLO11n（67 类精准版 / 102 类广谱版，可动态切换）
...
- **入场动画**：0.6s fade-up，使用 `--ease-out-expo` 缓动曲线
- **渐进式显示**：列表/卡片逐项递增延迟（0.1s 间隔）
- **悬浮反馈**：卡片上移 + 阴影增强 + 按压缩放

### 界面布局

- 全局 max-width 1040px 居中布局
- 检测页面：左侧控制栏 + 右侧内容区
- 所有页面统一视觉风格和交互反馈
...
| POST | `/api/detection/frame` | 摄像头帧检测 |
